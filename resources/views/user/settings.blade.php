...
<html>
    ...
    <article class="card-body reg-card">
        ...
        <hr>
        <form method="post" action="{{ action('App\Http\Controllers\UsersController@update') }}" accept-charset="UTF-8">
        ...
        <div class="input-group">
            <textarea name="" class="form-control" type="text" name="bio">{{ $bio }}</textarea>
        </div>
        </div> 
        <div class="form-group">
        ...
        <button type="submit" class="btn btn-primary card-btn mt-5 text-center"> Подтвердить </button>
        </div>
        </form>
    </article>
    </div>
